...
<?php
	...
	$md5filename = $uploaddir . rand() . md5("review_".$filename);
	...
	if ($upload_check) { // 업로드에 실패 시, INSERT or UPDATE를 진행하지 않는다.
		...
		{
			$input = "INSERT INTO review_board (Title, Content, File, File_en) VALUES('$title','$content','$filename','$md5filename')"; /*string일 때는 ''로묶어줘야한다*/
		}
		else { // ID 있음 -> 게시글 수정
			$result = mysqli_query($connection, "SELECT * FROM review_board WHERE ID=$ID");
			$row = mysqli_fetch_assoc($result);
			@unlink($row['File_en']);
			$input = "UPDATE review_board SET Title='$title', Content='$content', File='$filename', File_en='$md5filename' WHERE ID=$ID"; /*string일 때는 ''로묶어줘야한다*/
		}
