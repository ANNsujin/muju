...
<?php
	...
	if ($upload_check) { // 업로드에 실패 시, INSERT or UPDATE를 진행하지 않는다.
		...
		{
			$input = "INSERT INTO notice_board (Title, Content, File, File_en, Classify) VALUES('$title','$content','$filename','$md5filename','$classify')"; /*string일 때는 ''로묶어줘야한다*/
		}
		else { // ID 있음 -> 게시글 수정
			$result = mysqli_query($connection, "SELECT * FROM notice_board WHERE ID=$ID");
			$row = mysqli_fetch_assoc($result);
			@unlink($row['File_en']);
			$input = "UPDATE notice_board SET Title='$title', Content='$content', File='$filename', File_en='$md5filename', Classify='$classify' WHERE ID=$ID"; /*string일 때는 ''로묶어줘야한다*/
		}
