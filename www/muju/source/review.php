...
<?php
   ...
   $result = mysqli_query($connection, "SELECT * FROM review_board");
   $total = mysqli_num_rows($result); //전체 게시글의 수
   include("paging.php");
   /* 페이징 로드 */
   $currentLimit = ($onePage * $page) - $onePage; //몇 번째의 글부터 가져오는지
   $sqlLimit = ' limit ' . $currentLimit . ', ' . $onePage; //limit sql 구문
   $sql = 'SELECT * FROM review_board ORDER BY ID DESC' . $sqlLimit; //원하는 개수만큼 가져온다.
   $result = $connection->query($sql);
?>
	<div class="container">
		<div class="menu_nanum">
			<img src="/muju/image/menu7_1.png" arc="/muju/image/menu7_1.png">
			<span> 커뮤니티 >> 여행후기</span>
		</div>
		<hr class="menu_line">
		<div class="contents" style="margin-left:15px; margin-right:15px;">
			<table class="table table-striped table-hover" style="table-layout:fixed;">
				...
				<tr>
					...
					<td class="board_title text-cut"> <a href='/muju/source/review_view.php?ID=<?= $row['ID'] ?>'> <?= $row['Title'] ?> </a>
			...
			<div class="board_button">
				<a href="/muju/source/review_write.php" class="btn btn-info btn-sm">글쓰기</a>
			</div>
			<div class="pagination_center">
				<ul class="pagination">
					<?php if($page != 1) { //첫페이지 아니면 '처음' 활성화 ?>
						<li><a href="/muju/source/review.php?page=1">&laquo;</a></li>
					<?php } else { //첫페이지면 '처음' 비활성화 ?>
						<li class="disabled"><a href="#">&laquo;</a></li>
						<?php }
					if($currentSection != 1) { //첫 섹션이 아니면 '이전' 활성화 ?>
						<li><a href="/muju/source/review.php?page=<?=$prevPage?>"><</a></li>
					...
					for($i = $firstPage; $i <= $lastPage; $i++) {
						...
						<?php }
							...
							<li>
						<?php } ?>
						<a href="/muju/source/review.php?page=<?=$i?>"><?=$i?></a></li>
						<?php }
					if($currentSection != $allSection) { //마지막 섹션이 아니면 '다음' 활성화 ?>
						<li><a href="/muju/source/review.php?page=<?=$nextPage?>">></a></li>
					<?php } else { //마지막 섹션이면 '다음' 비활성화 ?>
						<li class="disabled"><a href="#">></a></li>
						<?php }
					if($page != $Pagenum) { // 마지막 페이지가 아니면 '끝' 활성화 ?>
						<li><a href="/muju/source/review.php?page=<?=$Pagenum?>">&raquo;</a></li>
